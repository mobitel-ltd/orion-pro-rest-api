...
const isValidEvent = (eventTypes: number[], accessPoints: number[]) => conforms({
...
const personHandler = (keyList: string[]) => pipe([
    pick(keyList),
    mapValues(consts.VALUE_NAME),
]);
...
export const getEvents = (eventTypes: number[], accessPoints: number[]) => pipe([
    head,
    get(consts.PATH_TO_INFO_ARRAY),
